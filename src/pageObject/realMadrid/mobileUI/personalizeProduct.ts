...
export class PersonalizeProduct {
  ...
  constructor(page: Page) {
    ...
    this.nameElement = page.locator('.swiper-wrapper div[style*="top: 21.66%"] p')
    this.numberElement = page.locator('.swiper-wrapper div[style*="top: 38.83%"] img')
  }
  ...
  async verifyPersonalizeOption() {
    ...
    await this.fillPersonalizeOptionDetails(
      productData.personalizeOptions.option1.name,
      productData.personalizeOptions.option1.number,
    )
    await this.product.addToCart()
    await this.checkout.checkoutButton.waitFor({ state: 'visible' })
    expect(await this.checkout.checkoutButton.isVisible()).toBeTruthy()
  ...
  async getRenderedNumber() {
    const numberImgs = await this.numberElement
