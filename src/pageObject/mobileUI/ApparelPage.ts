...
export default class ApparelPage {
  ...
  constructor(page: Page) {
    ...
    this.KitsButton = page.getByRole('button', { name: 'Kits' });
    this.selectKitsOption = page.getByRole('link', { name: 'Retro Kits' });
    ...
    this.addToCartButton = page.getByTestId('addtocartbutton').getByRole('button', { name: 'Add to Cart' })
    ...
    this.yourDetalsSection = page.locator('div:nth-child(3) > .p-6'); // This locator is fragile, consider improving if possible
  ...
  async clickFashionButton() {
    ...
    if (await this.KitsButton.isVisible()) {
      await this.KitsButton.click({force: true});
    }
    await this.selectKitsOption.click()
    }
