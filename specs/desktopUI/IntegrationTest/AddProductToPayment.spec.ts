...
import CommonFunction from '../../../src/pageObject/desktopUI/ApparelPage'
import testData from '../../../src/fixtures/validationData.json'

// test.setTimeout(150000); // Set a global timeout of 60 seconds for all tests

test.describe('Product Buying End-to-End Test', () => {

  test('should allow a user to buy product from the homepage to add in the cart then payment', async ({ page }) => {
    const apparelPage = new CommonFunction(page);
    ...
    await apparelPage.clickFashionLink();
    ...
    await apparelPage.fillContactDetails(testData.details.name, testData.details.email, testData.details.phone, testData.details.country);

    // Fill Shipping Address
    await apparelPage.fillShippingAddress(testData.address.street, testData.address.houseNumber, testData.address.city, testData.address.state, testData.address.zip);
